...
name: codex
description: |
  【Codex CLI 自身は使用しないこと / Claude など Codex 以外のエージェント専用】
...
> **このスキルを実行しているのがあなた自身 (Codex CLI) の場合は、使用してはならない。**
> これは Claude など「Codex 以外のエージェントが Codex CLI に相談する」ためのスキルであり、Codex が自分自身を呼び出すのは無意味な自己参照になる。あなたが Codex なら、このスキルは無視して通常どおり自力で回答すること。
