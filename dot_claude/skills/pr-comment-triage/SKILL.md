...
description: |
  ...
  - 対応不要 → 返信を投稿して thread resolve まで実行 (デフォルト。承認制にしたい場合は明示指示)
...
3. **「対応不要」は特段の指示がなければ reply 投稿 + resolve まで実行する** (判定一覧と返信本文はその場で提示する。承認待ちにするのは「判定のみ」「承認制で」など明示指示があるときだけ)
...
| モード | 用途 |
|---|---|
| **フル** (デフォルト) | 未解決スレッド全件を 1 周 triage し、「対応不要」は reply + resolve まで実行 |
| **単一** | 指定 thread id だけ処理 (reply + resolve まで実行) |
| **判定のみ** | reply / resolve は一切しない、判定結果だけ提示 |
| **承認制** | 返信案を提示して承認を待ってから reply / resolve |

特段の指示がなければ **フル** で動く。「判定のみ」「承認制」は、ユーザーがそう言ったときだけ選ぶ。
...
| **不明** | 文脈が読み取れない / 採否判断ができない | ユーザーに質問 (reply / resolve はしない) |
...
デフォルトでは「対応不要」と判定したものから順に実行する。承認制のときは承認が取れたものから順に実行。
...
- **判定一覧と返信本文を見せずに reply / resolve を実行する** (実行はするが、何を投稿したかは必ず可視化する)
- **「判定のみ」「承認制」と指示されているのに reply / resolve を実行する**
...
```
1. モード宣言 (デフォルト: フル / 指示があれば 単一・判定のみ・承認制)
   ↓
...
4. 判定ごとに分岐
   ...
   ├ 対応不要 → 返信本文を提示 → reply + resolve を実行 (承認制のときだけ承認を待つ)
   └ 不明     → ユーザーに質問
   ↓
5. 実行結果を 1 行で報告
```
